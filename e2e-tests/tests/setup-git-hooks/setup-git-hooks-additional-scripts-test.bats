...
@test "if pre-commit scripts exist 'committing' will execute them" {
	local scripts_path
	scripts_path="$(repository_dir)/hook-scripts/pre-commit"
...
@test "if commit-msg scripts exits with failure 'commiting' will also fail" {
	local script_path
	script_path="$(repository_dir)/hook-scripts/commit-msg/script"
...
@test "if pre-commit hook gets executed, it gets passed the git parameters" {
	local script_path
	script_path="$(repository_dir)/hook-scripts/pre-commit/script"
...
@test "if pre-push hook gets executed, it gets passed the git parameters" {
	local script_path
	script_path="$(repository_dir)/hook-scripts/pre-push/script"
	write_spy_script "$script_path"
	local branch_name
	...
	local origin_url
	origin_url=$(git -C "$(repository_dir)" config --get remote.origin.url)
	local remote_name
